patch -Np2 -d /work/build/linux-lts < /work/package/linux-lts/patch/PKGBUILD.patch
[[ $? -gt 0 ]] && exit 1
sed -i \
-e '/pkgver=.*/a _kernelversion="$(echo ${pkgver} | cut -d '.' -f 1,2)"' \
-re 's/pkgbase=linux-lts.*/pkgbase=linux-lts-15khz/' \
/work/build/linux-lts/repos/core-x86_64/PKGBUILD || exit 1
( cd /work/build/linux-lts/repos/core-x86_64 && makepkg -g >> PKGBUILD )

# Simply build DRM
sed -i 's+make bzImage modules htmldocs+make -C . M=drivers/gpu/drm+g' /work/build/linux-lts/repos/core-x86_64/PKGBUILD || exit 1
